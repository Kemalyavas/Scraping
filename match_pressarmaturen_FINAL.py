...
def calculate_match_score(heizmann_item, balflex_item):
    """
    Calculate match score with STRICT angle and standard checking
    ...
    # === CRITERION 1: ANGLE (MANDATORY - 25 points) ===
    ...
    # === CRITERION 2: STANDARD (MANDATORY - 35 points) ===
    ...
    # === CRITERION 4: SEAT TYPE (10 points) ===
...
def match_products(heizmann_data, balflex_data, min_threshold=60):
    """
    Match products with strict angle and standard checking
    min_threshold: Minimum score to consider a match (default 60)
    """
    print(f"\nMatching products with minimum threshold: {min_threshold}...")
    print(f"STRICT MODE: Angle and Standard must match!")

    matches = []
    rejected_counts = {'angle_mismatch': 0, 'standard_mismatch': 0, 'low_score': 0}
...
def analyze_and_save_matches(matches, heizmann_total, balflex_total):
    ...
    output_file = 'data/balflex_heizmann_PRESSARMATUREN_FINAL.xlsx'
...
def main():
    """Main execution"""
    print("="*80)
    print("FINAL PRODUCT MATCHING ALGORITHM - CORRECTED VERSION")
    print("="*80)
