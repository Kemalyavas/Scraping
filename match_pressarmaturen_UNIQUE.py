...
def match_products(heizmann_data, balflex_data, min_threshold=60):
    """
    Match products with 1:1 unique matching (no duplicates)
    Uses greedy algorithm: highest score matches first
    ...
    print(f"UNIQUE MODE: Each product can match only ONCE (1:1 matching)!")

    # Step 1: Calculate ALL possible matches
    print("\n1. Calculating all possible matches...")
        ...
        for balflex_item in balflex_data:
            ...
            if score >= min_threshold:
                all_potential_matches.append({
                    'heizmann': heizmann_item,
                    'balflex': balflex_item,
                    'score': score,
                    'reasons': reasons
                })

    print(f"   Found {len(all_potential_matches)} potential matches (score >= {min_threshold})")

    # Step 2: Sort by score (highest first)
    print("\n2. Sorting by score...")
    all_potential_matches.sort(key=lambda x: x['score'], reverse=True)

    # Step 3: Greedy selection - ensure 1:1 matching
    print("\n3. Selecting unique 1:1 matches (greedy algorithm)...")
    used_heizmann = set()
    used_balflex = set()
    final_matches = []

    for potential in all_potential_matches:
        heiz_id = potential['heizmann']['article_number']
        balf_id = potential['balflex']['article_number']

        # Skip if either product is already matched
        if heiz_id in used_heizmann or balf_id in used_balflex:
            continue

        # Add this match
        used_heizmann.add(heiz_id)
        used_balflex.add(balf_id)
        final_matches.append(potential)

    print(f"   Selected {len(final_matches)} unique 1:1 matches")
    print(f"   Rejected {len(all_potential_matches) - len(final_matches)} duplicate matches")

    # Step 4: Format matches for Excel output
    print("\n4. Formatting results...")
    matches = []

    for match in final_matches:
        heizmann_item = match['heizmann']
        balflex_item = match['balflex']
        best_score = match['score']
        best_reasons = match['reasons']

        best_match = balflex_item  # For compatibility with rest of code

        if True:  # Always true, just to keep indentation
...
def analyze_and_save_matches(matches, heizmann_total, balflex_total):
    ...
    output_file = 'data/balflex_heizmann_PRESSARMATUREN_UNIQUE.xlsx'
